...
PLACE_OPTIONS_ORIG_TEXT = "地點僅接受：1. MAYDAY LAND  2. 洲際棒球場，可直接輸入代號"
PLACE_OPTIONS_DESIRED_TEXT = "地點僅接受：1. MAYDAY LAND  2. 洲際棒球場  3. 皆可，可直接輸入代號"
...
def place_options_text(key: str) -> str:
    return PLACE_OPTIONS_DESIRED_TEXT if key == "desired_place" else PLACE_OPTIONS_ORIG_TEXT
...
def validate_field(key: str, value: str, data: Optional[Dict[str, str]] = None) -> Tuple[Optional[str], Optional[str]]:
    ...
    if key in {"orig_place", "desired_place"}:
        ...
        if normalized is None:
            return None, f"{label_with_hint(key)}，{place_options_text(key)}。"
...
def build_help_message() -> str:
    return (
        ...
        "- 輸入「解除 我的訂單編號 對方訂單編號」，解除已配對的資料並回到待配對，例如:解除 987654321 552510329。\n"
...
def handle_message(event):
    ...
    if text.startswith("解除"):
        ...
        parts = text.split()
        if len(parts) < 3:
            reply = "解除請輸入：解除 我的訂單編號 對方訂單編號，例如:解除 987654321 552510329。"
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text=reply))
            return
